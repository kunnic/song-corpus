...
class EmolexWord:
    ...
    def is_negative(self) -> bool:
        '''
            - return True if the sentimental value is negative.
            - args:
                + output: bool value if it's negative or else.
        '''
        return bool(self.sentiments & np.uint8(2))
...
# ───────────────────────────────────────────────────────────────────
@dataclass(slots = True, frozen = True)
class EmolexDictionary:
    word_to_index:  dict[str, int]
    matrix:         np.ndarray
    emotions:       list[str]

    def get_index(self, tokens: list[str]) -> list[int]:
        return [
            self.word_to_index[token]
            for token in tokens if token in self.word_to_index
        ]
